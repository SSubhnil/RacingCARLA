...
def Curvature(s, PointAndTangent):
    ...
    """
    TrackLength = PointAndTangent[-1,3]
    ...
    index = (np.abs(PointAndTangent[:,3] - s)).argmin()
    curvature = PointAndTangent[index, 4]
    return curvature
...
def getAngle(s, epsi, PointAndTangent):
    ...
    # index = np.argwhere(abs(PointAndTangent[:,3] - s) <= 0.005)
    # i = min(index)
    index = (np.abs(PointAndTangent[:,3] - s)).argmin()
    curvature = PointAndTangent[index, 4]
    ...
    if PointAndTangent[i, 4] == 0:
        r= 0
    else:
        r = 1 / PointAndTangent[i, 4]  # Radius of curvature
